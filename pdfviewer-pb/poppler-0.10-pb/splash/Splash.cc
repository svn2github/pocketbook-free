  ...
  if (pipe->noTransparency && !state->blendFunc) {
	...
	*pipe->destColorPtr &= ~pipe->destColorMask;
    ...
    case splashModeMono4:
      cResult0 = pipe->cSrc[0];
      if (pipe->destColorMask & 0xf0) {
        *pipe->destColorPtr &= 0x0f;
        *pipe->destColorPtr |= (cResult0 & 0xf0);
      } else {
        *pipe->destColorPtr &= 0xf0;
        *pipe->destColorPtr |= ((cResult0 >> 4) & 0xf);
      }
      if (!(pipe->destColorMask >>= 4)) {
	pipe->destColorMask = 0xf0;
	++pipe->destColorPtr;
      }
      break;
    ...
    case splashModeMono1:
      ...
      break;
    case splashModeMono4:
      cResult0 = *pipe->destColorPtr;
      cResult0 = ((pipe->destColorMask & 0xf0) ? (cResult0 >> 4) : cResult0) & 0xf;
      cDest[0] = cResult0 | (cResult0<<4);
      break;
    case splashModeMono8:
...
inline void Splash::pipeSetXY(SplashPipe *pipe, int x, int y) {
  ...
  case splashModeMono1:
    ...
    break;
  case splashModeMono4:
    pipe->destColorPtr = &bitmap->data[y * bitmap->rowSize + (x >> 1)];
    pipe->destColorMask = (x & 1) ? 0xf : 0xf0;
    break;
  case splashModeMono8:
...
void Splash::clear(SplashColorPtr color, Guchar alpha) {
  ...
  case splashModeMono1:
    ...
    break;
  case splashModeMono4:
    mono = (color[0] & 0xf0) | ((color[0] >> 4) & 0xf);
    if (bitmap->rowSize < 0) {
      memset(bitmap->data + bitmap->rowSize * (bitmap->height - 1),
	     mono, -bitmap->rowSize * bitmap->height);
    } else {
...
void Splash::compositeBackground(SplashColorPtr color) {
  ...
  case splashModeMono4:
	...
	c = ((x & 1) ? *p : (*p >> 4)) & 0xf;
	c = c | (c<<4);
	p[0] = div255(alpha1 * color0 + alpha * c);
	if (x & 1) ++p;
      }
    }
    break;
...
SplashError Splash::blitTransparent(SplashBitmap *src, int xSrc, int ySrc,
  ...
  case splashModeMono4:
    for (y = 0; y < h; ++y) {
      p = &bitmap->data[(yDest + y) * bitmap->rowSize + (xDest >> 1)];
      mask = (xDest & 1) ? 0xf : 0xf0;
      for (x = 0; x < w; ++x) {
	src->getPixel(xSrc + x, ySrc + y, pixel);
	if (mask & 0xf0) {
	  *p = (*p & 0x0f) | (pixel[0] & 0xf0);
	} else {
	  *p = (*p & 0xf0) | ((pixel[0] >> 4) & 0x0f);
	}
	if (!(mask >>= 4)) {
	  mask = 0xf0;
	  ++p;
