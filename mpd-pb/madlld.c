...
static int MpegAudioDecoder(FILE *InputFp, FILE *OutputFp)
{
	 ...
	 */
	if(OutputPtr!=OutputBuffer && Status!=2 && mpc->state == MP_PLAYING)
	{
...
int random_next(int n) {
	...
	for (i=0; i<mpc->plcount; i++) {
		if (randoms[i] == n)
			return randoms[(i+1) % mpc->plcount];
	}
	return 0;

}

int random_prev(int n) {

	int i;

	for (i=0; i<mpc->plcount; i++) {
		if (randoms[i] == n)
			return randoms[(i+mpc->plcount-1) % mpc->plcount];
	}
	return 0;
...
int main(int argc, char **argv)
{
	...
	mp3_shm = shmget(0xa12302, sizeof(iv_mpctl), IPC_CREAT | 0666);
	if (mp3_shm == -1) {
		fprintf(stderr, "cannot get shared memory segment\n");
		return 1;
	}
	mpc = (iv_mpctl *) shmat(mp3_shm, NULL, 0);
	if (mpc == (void*)-1) {
	...
	while (1) {
		...
		while (mpc->state == MP_STOPPED) usleep(50000);

		if (mpc->state == MP_REQUEST_FOR_PLAY) {
			fprintf(stderr, "request for play: track %i\n", mpc->track);
			srand(time(NULL));
			if (randoms) free(randoms);
			randoms = (short *) malloc(mpc->plcount * sizeof(short));
			for (i=0; i<mpc->plcount; i++) randoms[i] = i;
			for (i=0; i<mpc->plcount; i++) {
				j = rand() % mpc->plcount;
				v = randoms[i];
				randoms[i] = randoms[j];
		...
		filename = mpc->plstrings + mpc->playlist[mpc->track];
		if (filename == NULL) {
		...
		if (mpc->state == MP_PREVTRACK) {
			if (mpc->mode != MP_RANDOM) {
				mpc->track--;
				if (mpc->track < 0) mpc->track = mpc->plcount-1;
		...
		if (mpc->state == MP_NEXTTRACK) {
			if (mpc->mode != MP_RANDOM) {
				mpc->track++;
				if (mpc->track >= mpc->plcount) mpc->track = 0;
			} else {
