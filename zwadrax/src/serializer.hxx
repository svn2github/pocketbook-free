...
private:
    ...
    struct line_type: public string
    {
        line_type(void) {}
        line_type(const string &src): string(src) {}
        friend
        istream & operator >>(istream &is, line_type &str)
        {
            getline(is, static_cast<string &>(str));
            if (!str.empty() && str[str.size() - 1] == '\r')
                str.resize(str.size() - 1);
            return is;
        }
        friend
        ostream & operator <<(ostream &os, const line_type &str)
        {
            os << static_cast<const string &>(str) << endl;
            return os;
        }
    };
    ...
    stringlist_type file2list(const string &filename)
    {
        ...
        return stringlist_type(
            istream_iterator<line_type>(ifile),
            istream_iterator<line_type>());
    }
    static
    void list2file(const stringlist_type &strlist, const string &filename)
    {
        ...
        copy(
            strlist.begin(),
            strlist.end(),
            ostream_iterator<line_type>(ofile));
    }
    ...
    void map2file(const stringmap_type &strmap, const string &filename)
    {
        ...
        {
            static
            string join(const pair<string, string> &p)
            {
                return p.first + '=' + p.second;
            }
        };
        transform(
            strmap.begin(), strmap.end(),
...
public:
    ...
    // string to integer and vice versa transformations
    static
