...
private:
    std::list<type> m_obj;
public:
    deferred_type & operator =(const type &obj)
    {
        m_obj.clear();
        m_obj.push_back(obj);
        return *this;
...
private:
    ...
    int m_lastmenu;
    // drawing queue timeout in milliseconds
    int m_refreshrate;
    // control rectangles
    ...
    dev_type device(void) const
    {
        struct { dev_type code; const char *name; } dev[] = {
            { DEV_360, "PocketBook 101" },
            { DEV_301, "PocketBook 301 plus" },
            { DEV_302, "PocketBook 302" },
            { DEV_602, "PocketBook 602" },
            { DEV_603, "PocketBook 603" },
            { DEV_902, "PocketBook 902" },
            { DEV_903, "PocketBook 903" }
        };
        const char *device = ::GetDeviceModel();
        dev_type rc = DEV_UNKNOWN;
        for (int i = 0; i < int(sizeof(dev) / sizeof(dev[0])); i++)
        {
            if (device && string(device) == string(dev[i].name))
                rc = dev[i].code;
        };
        return rc;
    };
    ...
    void menumain(void)
    {
        ...
        bool gamewon = m_game.empty() || m_game.ref().iswon();
        // game or edit mode is on
        bool ingame = !m_game.empty() && !m_game.ref().isedit();
        bool inedit = !m_game.empty() && m_game.ref().isedit();
        ...
        bool play = true)
    {
        ...
        m_viewport = viewport_type(
            m_game.ref().width(), m_game.ref().height(),
            maxwidth(), maxheight());
        fullredraw();
        drawqueue();
    ...
    void store(void)
    {
        ...
        if (!m_game.empty())
        {
            cfg["mode"] = m_game.ref().isedit() ? "edit" : "game";
            if (m_game.ref().isedit())
            {
                // store level
    ...
    void drawqueue(void)
    {
        // draw queue head if it isn't already drawing
        if (::QueryTimer(timer_handler) == 0)
        {
            drawpack_type pack;
            // get next drawpack
            if (m_game.ref().dequeue(pack))
            {
            ...
            else
            {
                ...
                if (m_game.ref().iswon() || m_game.ref().isover())
                    menumain();
    ...
    {
        // tick after stated time
        ::SetHardTimer("zwtimer", timer_handler, m_refreshrate);
    }
    ...
    bool translate(int type, int par1, int par2, evt_type &evt)
    {
        ...
        case EVT_KEYRELEASE:
            if (par2 == 0)
                ...
                // main menu hotkey (301, 302)
                case KEY_BACK:
                    evt = MENU;
                    break;

                // shift menu hotkey (301)
                case KEY_DELETE:
                    evt = MENU3X3;
                    break;

                // shift menu hotkey (360, 60x, 90x) or left key (302)
                case KEY_PREV:
                    evt = (device() != DEV_302) ? MENU3X3 : LEFT;
                    break;
                // main menu hotkey (360, 60x, 90x) or right key (302)
                case KEY_NEXT:
                    evt = (device() != DEV_302) ? MENU : RIGHT;
                    break;

                // up and down keys (302)
                case KEY_PREV2:
                    evt = UP;
                    break;
                case KEY_NEXT2:
        ...
        case EVT_KEYREPEAT:
            if (par2 == 1)
                switch (par1)
                {
                // game key (360, 301, 60x, 90x)
                case KEY_OK:
                    evt = ACTIVATE;
                    break;

                // shift menu key (360, 301, 60x, 90x)
                case KEY_UP:
                    evt = MENU3X3;
                    break;

                // main menu key (360, 301, 60x, 90x)
                case KEY_DOWN:
                    evt = MENU;
                    break;
                ...
                // game key (302)
                case KEY_PREV:
                    evt = (device() != DEV_302) ? NO_EVT : ACTIVATE;
                    break;
                // exit hotkey (360, 60x, 90x) or game key (302)
                case KEY_NEXT:
                    evt = (device() != DEV_302) ? EXIT : SWITCH;
                    break;

                // shift menu key and exit hotkey (302)
                case KEY_PREV2:
                    evt = MENU3X3;
                    break;
                case KEY_NEXT2:
            ...
            case EVT_INIT:
                {
                    ...
                    DPRINTF("device model code %d", device());
                    ...
                    // load and erase main config
                    stringmap_type cfg = m_serializer.load_config();
                    m_serializer.erase_config();
                    // set stored or root levelset
                    m_levelset = cfg["levelset"];
                    if (m_levelset.empty())
                        m_levelset = m_serializer.levelsets().front();
                    // load refresh rate or set to default
                    m_refreshrate = serializer_type::str2int(cfg["refreshrate"]);
                    m_refreshrate = (m_refreshrate != 0) ? m_refreshrate : 200;
                    // restore game status
                    restore();
                };
                break;
            ...
            case EVT_EXIT:
                {
                    ...
                    cfg["refreshrate"] = serializer_type::int2str(m_refreshrate);
        ...
        else
        {
            ...
            if (translate(type, par1, par2, evt))
                ...
                case ACTIVATE:
                    ...
                    {
                        // give it to game
                        m_game.ref().process(game_type::key_type(evt));
                        drawqueue();
                    };
                    break;
        ...
        case MENU_NEXT:
            // find and start next level
            {
                ...
                set_difference(
                    ...
                    inserter(nonpassed, nonpassed.end()));
    ...
    void handlekeyboard(const char *str)
    {
        ...
        if (str)
        {
            ...
            if (serializer_type::iscorrect(str))
            {
                ...
                {
                    m_level = str;
                    m_serializer.save_level(m_game.ref().data(), m_levelset, m_level);
                };
                // or save solution
                if (m_lastmenu == MENU_SAVESOLUTION)
                    m_serializer.save_solution(m_game.ref().sequence(), m_levelset, m_level, str);
            }
            else
