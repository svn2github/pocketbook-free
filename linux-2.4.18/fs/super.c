...
static void remove_super(struct super_block *s)
{
	...
	spin_lock(&sb_lock);
	list_del(&s->s_list);
	//printk("remove super 1 \n");
	list_del(&s->s_instances);
	//printk("remove super 2 \n");
	spin_unlock(&sb_lock);
	up_write(&s->s_umount);
	//printk("remove super 3 \n");
	put_super(s);
	//printk("remove super 4 \n");
	put_filesystem(fs);
	//printk("remove super 5 \n");
	if (bdev)
	{
		//printk("remove super 6 \n");
		blkdev_put(bdev, BDEV_FS);
	}else
	{
		//printk("remove super 7 \n");
		put_unnamed_dev(dev);
	}
	//printk("remove super 8 \n");
}

static void remove_super2(struct super_block *s)
{
	kdev_t dev = s->s_dev;
	struct block_device *bdev = s->s_bdev;
	struct file_system_type *fs = s->s_type;
	//printk("remove super2 \n");
	spin_lock(&sb_lock);
	list_del(&s->s_list);
	//printk("remove super2 1 \n");
	list_del(&s->s_instances);
	//printk("remove super2 2 \n");
	spin_unlock(&sb_lock);
	up_write(&s->s_umount);
	//printk("remove super2 3 \n");
	put_super(s);
	//printk("remove super2 4 \n");
	put_filesystem(fs);
	//printk("remove super2 5 \n");
	if (bdev)
	{
		//printk("remove super 6 \n");
		blkdev_put2(bdev, BDEV_FS);
	}else
	{
		//printk("remove super 7 \n");
		put_unnamed_dev(dev);
...
void kill_super2(struct super_block *sb)
{
	struct dentry *root = sb->s_root;
	struct file_system_type *fs = sb->s_type;
	struct super_operations *sop = sb->s_op;
	//printk("into kill_super2. \n");
	if (!deactivate_super(sb))
		return;
	//printk("kill_super2 1 \n");
	down_write(&sb->s_umount);
	//printk("kill_super2 2 \n");
	lock_kernel();
	sb->s_root = NULL;
	/* Need to clean after the sucker */
	if (fs->fs_flags & FS_LITTER)
		d_genocide(root);
	//printk("kill_super2 3 \n");	
	shrink_dcache_parent(root);
	//printk("kill_super2 4 \n");
	dput(root);
	//printk("kill_super2 5 \n");
	//fsync_super(sb);
	//printk("kill_super2 6 \n");
	lock_super(sb);
	//printk("kill_super2 7 \n");
	sb->s_flags &= ~MS_ACTIVE;
	invalidate_inodes(sb);	/* bad name - it should be evict_inodes() */
	//printk("kill_super2 8 \n");
	/*if (sop) {
		printk("kill_super2 9 \n");
		if (sop->write_super && sb->s_dirt)
		{
			printk("kill_super2 10 \n");
			sop->write_super(sb);
		}
		if (sop->put_super)
		{
			printk("kill_super2 11 \n");
			sop->put_super(sb);
		}
	}*/
	//printk("kill_super2 12 \n");
	/* Forget any remaining inodes */
	if (invalidate_inodes(sb)) {
		printk("VFS: Busy inodes after unmount. "
			"Self-destruct in 5 seconds.  Have a nice day...\n");
		//printk("kill_super2 13 \n");	
	}
	
	unlock_kernel();
	//printk("kill_super2 14 \n");
	unlock_super(sb);
	//printk("kill_super2 15 \n");
	remove_super2(sb);
	//printk("kill_super2 16 \n");
}
...
void kill_super(struct super_block *sb)
{
	...
	if (sop) {
		...
		if (sop->write_super && sb->s_dirt)
		{
			//printk("kill_super 10 \n");
			sop->write_super(sb);
		}
		if (sop->put_super)
		{
			//printk("kill_super 11 \n");
			sop->put_super(sb);
		}
	}
	//printk("kill_super 12 \n");
	/* Forget any remaining inodes */
	if (invalidate_inodes(sb)) {
		printk("VFS: Busy inodes after unmount. "
			"Self-destruct in 5 seconds.  Have a nice day...\n");
		//printk("kill_super 13 \n");	
	}
	
	unlock_kernel();
	//printk("kill_super 14 \n");
	unlock_super(sb);
	//printk("kill_super 15 \n");
	remove_super(sb);
	//printk("kill_super 16 \n");
}
