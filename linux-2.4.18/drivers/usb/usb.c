...
//	usb_reset_device(dev); //  by dz
	...
	usb_free_dev(dev);
#else
	printk("dz: don't disconnect usb \n");
...
extern ohci_t *s3c2410_ohci;

void usb_clk_on3(void)
{
...
//Joe
//        printk(KERN_ALERT" usb.c UPLLCON = %x MISCCR = %x\n",UPLLCON,MISCCR);
...
//		printk(KERN_ALERT"usb err!\n");				
		usb_clk_on3();		
		wait_ms(2+retrycount*2);
		err=usb_set_address(dev);				
		
		if (err>=0)
		{
			printk(KERN_ALERT"reset addr ok!!! \n");
			break;
		}					
...
#ifdef CONFIG_PM
struct pm_dev *usb_pm_dev;

 
static int
usb_pm_callback(struct pm_dev *pm_dev, pm_request_t req, void *data)
{
	int pid;

	if (req == PM_SUSPEND) {
		printk("dz: usb.c suspend \n");			
		
	} else if (req == PM_RESUME) 
	{
		printk("dz: usb.c resume \n");				

	}else
	{
		printk("dz: usb.c else \n");	
...
#if 0
	usb_pm_dev = pm_register(PM_USER_DEV, 0,
					 usb_pm_callback);
