...
void ep_print_int_eds (ohci_t * ohci, char * str) {
	...
	for (i= 0; i < 32; i++) {
		...
		if (*ed_p == 0)
		    continue;
		printk ( __FILE__ ": %s branch int %2d(%2x):", str, i, i);
		while (*ed_p != 0 && j--) {
...
{
	printk ("%s: 0x%08x%s%s%s%s%s%s%s%s%s\n",
		label,
		mask,
...
#ifdef CONFIG_ARCH_S3C2410
	if ((ohci->rh.devnum == 1) // Root Hub
			&& (num_ports > CONFIG_MAX_ROOT_PORTS))
		num_ports = CONFIG_MAX_ROOT_PORTS;
#endif
...
static void hc_interrupt (int irq, void * __ohci, struct pt_regs * k)
{
	ohci_t * ohci = __ohci;
...
//	printk("Interrupt: %x frame: %x", ints, le16_to_cpu (ohci->hcca->frame_no));

	if (ints & OHCI_INTR_UE) {
		ohci->disabled++;
		printk ("OHCI Unrecoverable Error, controller usb-%s disabled",
			ohci->slot_name);
...
ohci_t * __devinit hc_alloc_ohci (struct pci_dev *dev, void * mem_base)
{
	ohci_t * ohci;
	struct usb_bus * bus;
...
int dz_ohci_suspend (ohci_t *ohci)
{
	unsigned long		flags;
	u16 cmd;

	if ((ohci->hc_control & OHCI_CTRL_HCFS) != OHCI_USB_OPER) {
		printk("can't suspend usb\n");
		return -EIO;
	}
...
int dz_ohci_resume (ohci_t	*ohci)
{
	int		temp;
	unsigned long	flags;
	...
	if (atomic_read (&ohci->resume_count) != 1) {
		printk("concurrent resumes for usb");
	...
	case OHCI_USB_RESET:	// lost power
		printk ("USB restart: usb");
		hc_restart (ohci);
		break;

	case OHCI_USB_SUSPEND:	// host wakeup
	case OHCI_USB_RESUME:	// remote wakeup
		printk ("USB continue: usb from %s wakeup",(temp == OHCI_USB_SUSPEND)? "host" : "remote");
		...
		if (temp != OHCI_USB_RESUME) {
			printk ("controller usb won't resume");
			ohci->disabled = 1;
			return -EIO;
		}
