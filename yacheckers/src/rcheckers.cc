...
bool RCheckers::go2_human(int from,int field)
{
    from=internal(from);
    field=internal(field);
    ...
    if(checkCapture2()){
        ...
        switch(board[from]){
        case MAN2:
            if(manCapture2(from,UL,capture)) return true;
            if(manCapture2(from,UR,capture)) return true;
...
// for 2 human players
bool RCheckers::manCapture2(int from,int direction,bool &capture)
{
  int i=from+direction;
  if(board[i]==MAN1 || board[i]==KING1){
    ...
    if(board[k]==FREE){
      ...
      if(k>42){//if(k<10){//Is it True?
        ...
        board[k]=MAN2;
        if(direction==UL || direction==DR){
          if(manCapture2(k,UR,next)) {board[i]=FREE;return true;}
          if(manCapture2(k,DL,next)) {board[i]=FREE;return true;}
        }else{
          if(manCapture2(k,UL,next)) {board[i]=FREE;return true;}
          if(manCapture2(k,DR,next)) {board[i]=FREE;return true;}
        }
        if(manCapture2(k,direction,next)) {board[i]=FREE;return true;}
      }
...
bool RCheckers::kingCapture2(int from,int direction,bool &capture)
{
    int i;
    for(i=from+direction;board[i]==FREE;i+=direction);
    ...
    if(board[i]==MAN1 || board[i]==KING1)
    {
        ...
        if(board[k]==FREE)
        {
            ...
            if(!next)
                for(;k!=i;k-=direction)
                    if(k==to) {board[i]=FREE;board[k]=KING2;return true;}
