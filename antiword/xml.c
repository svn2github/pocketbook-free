...
/* Text is emphasised */
static BOOL	bEmphasis = FALSE;
static BOOL	bEmphasisOpen = FALSE;
static BOOL	bEmphasisBold = FALSE;
static BOOL	bEmphasisItalic = FALSE;
static BOOL	bEmphasisUnderline = FALSE;
/* Text is superscript */
...
static const docbooktags_type atDocBookTags[] = {
	...
	{	TAG_AUTHOR,		"!Tag author",	TRUE,	TRUE	},
	{	TAG_BEGINPAGE,		"!Tag begin",	TRUE,	TRUE	},
	{	TAG_BOOK, 		"HTML",		TRUE,	TRUE	},
	{	TAG_BOOKINFO, 		"!Tag bookinfo",	TRUE,	TRUE	},
	{	TAG_CHAPTER, 		"BODY",	TRUE,	TRUE	},
	{	TAG_COLSPEC,		"!Tag colspec",	TRUE,	TRUE	},
	{	TAG_CORPNAME,		"!Tag corpname",	FALSE,	FALSE	},
	{	TAG_DATE,		"!Tag date",		FALSE,	FALSE	},
	{	TAG_EMPHASIS,		"!x",	FALSE,	FALSE	},
	{	TAG_ENTRY,		"td",	TRUE,	TRUE	},
	{	TAG_FILENAME,		"!Tag filename",	FALSE,	FALSE	},
	{	TAG_FOOTNOTE,		"!Tag footnote",	FALSE,	FALSE	},
	{	TAG_INFORMALTABLE,	"!Tag informal",TRUE,	TRUE	},
	{	TAG_ITEMIZEDLIST,	"!Tag itemized",	TRUE,	TRUE	},
	{	TAG_LISTITEM,		"!Tag listitem",	TRUE,	TRUE	},
	{	TAG_ORDEREDLIST,	"!Tag ordered",	TRUE,	TRUE	},
	{	TAG_PARA, 		"p",		TRUE,	TRUE	},
	{	TAG_ROW,		"tr",		TRUE,	TRUE	},
	{	TAG_SECT1, 		"!Tag sect1",	TRUE,	TRUE	},
	{	TAG_SECT2, 		"!Tag sect2",	TRUE,	TRUE	},
	{	TAG_SECT3, 		"!Tag sect3",	TRUE,	TRUE	},
	{	TAG_SECT4, 		"!Tag sect4",	TRUE,	TRUE	},
	{	TAG_SECT5, 		"!Tag sect5",	TRUE,	TRUE	},
	{	TAG_SUBSCRIPT,		"!Tag subscript",	FALSE,	FALSE	},
	{	TAG_SUBTITLE,		"!Tag subtitle",	FALSE,	FALSE	},
	{	TAG_SUPERSCRIPT,	"!Tag superscr",	FALSE,	FALSE	},
	{	TAG_SURNAME,		"!Tag surname",	FALSE,	FALSE	},
	{	TAG_TBODY,		"!Tag tbody",	TRUE,	TRUE	},
	{	TAG_TGROUP,		"TABLE",	TRUE,	TRUE	},
	{	TAG_TITLE, 		"!Tag title",	FALSE,	FALSE	},
};
...
vAddStartTag(diagram_type *pDiag, UCHAR ucTag, const char *szAttribute)
{
	...
	if (szAttribute == NULL || szAttribute[0] == '\0') {
		...
		if ((!(bEmphasis==TRUE))&&(ucTag!=TAG_TITLE) && (ucTag!=TAG_BOOK))
			fprintf(pDiag->pOutFile, "<%s %s>",atDocBookTags[(UINT)ucTag].szTagname, szAttribute);
	}
	...
	case TAG_PARA:
		...
		break;
	case TAG_EMPHASIS:
		if (bEmphasisBold==TRUE)
			fprintf(pDiag->pOutFile,"<B>");
		if (bEmphasisItalic==TRUE)
			fprintf(pDiag->pOutFile,"<I>");
		if (bEmphasisUnderline==TRUE)
			fprintf(pDiag->pOutFile,"<U>");
		bEmphasisOpen = TRUE;
		break;
...
/*	if (ucTag==TAG_CHAPTER)
		fprintf(pDiag->pOutFile, "</BODY>\n");*/
	if ((!(bEmphasis==TRUE))&&(ucTag!=TAG_TITLE)&&(ucTag!=TAG_TBODY)&&(ucTag!=TAG_INFORMALTABLE))
		fprintf(pDiag->pOutFile, "</%s>", atDocBookTags[(UINT)ucTag].szTagname);
	...
	case TAG_EMPHASIS:
		if (bEmphasisUnderline==TRUE)
			{
			 bEmphasisUnderline=FALSE;
			 fprintf(pDiag->pOutFile,"</U>");
			}
		if (bEmphasisItalic==TRUE)
			{
			 bEmphasisItalic=FALSE;
			 fprintf(pDiag->pOutFile,"</I>");
			}
		if (bEmphasisBold==TRUE)
			{
...
/*	if (szLanguage != NULL) {
	...
	fprintf(pDiag->pOutFile,"<!-- Create for PocketBook -->\n");
		...
		USHORT usFontstyle)
{
	...
	if (bEmphasisNew && !bEmphasisOpen) {
		...
		if (bIsItalic(usFontstyle)) {
			bEmphasis=TRUE;
			bEmphasisItalic=TRUE;
		}
		if (bIsUnderline(usFontstyle)) {
			bEmphasis=TRUE;
			bEmphasisUnderline=TRUE;
		}
/*		if (bIsStrike(usFontstyle)) {
			szAttr = NULL;
//			bEmphasis=TRUE;
		}*/
...
/*		vAddCombinedTag(pDiag, TAG_TITLE, NULL);
	...
	vAddStartTag(pDiag, TAG_INFORMALTABLE, "");
