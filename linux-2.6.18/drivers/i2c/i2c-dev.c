...
/* $Id: i2c-dev.c,v 1.2 2004/02/06 13:19:45 laputa Exp $ */
...
#endif
//laputa unsigned char for iic data format 030903
//static ssize_t i2cdev_read (struct file *file, char *buf, size_t count,  // -- remove 
static ssize_t i2cdev_read (struct file *file, unsigned char *buf, size_t count,  // +- modify 
                            loff_t *offset);
//laputa unsigned char for iic data format 030903 
//static ssize_t i2cdev_write (struct file *file, const char *buf, size_t count, //--remove 
static ssize_t i2cdev_write (struct file *file, const unsigned char *buf, size_t count,  //+-modify
                             loff_t *offset);
...
//laputa must be unsgined char type for the iic data 030903
//static ssize_t i2cdev_read (struct file *file, char *buf, size_t count,
static ssize_t i2cdev_read (struct file *file, unsigned char *buf, size_t count,
                            loff_t *offset)
{
	//char *tmp;     	// laputa -- remove
	unsigned char *tmp; // laputa +- modify 030903
	int ret;
	...
	DbgMsg(LAPUTA_DEBUG_MSG,printk("--> reading %d bytes.\n",count));
...
static ssize_t i2cdev_write (struct file *file, const unsigned char *buf, size_t count,
                             loff_t *offset)
{
	int ret;
	// char *tmp; 			//--remove 
	unsigned char *tmp;     // +- modify 030903 
	struct i2c_client *client = (struct i2c_client *)file->private_data;
	...
	//laputa check for buf chip addres setting & transfer data 030901
	DbgMsg(LAPUTA_DEBUG_MSG,printk("==>i2c dev  write buf [%s] addr[%x] \n",buf,client->addr));
                  ...
                  unsigned long arg)
{
	...
	//laputa check for argument at ioctl 030830
	DbgMsg(LAPUTA_DEBUG_MSG,printk("client->name:%s:arg=%x\n",client->name,arg));
	...
	//laputa for to checked a address setting value 030830
	DbgMsg(LAPUTA_DEBUG_MSG,printk("==>I2C_SLAVE_FORCE ADD [%x]\n",client->addr));
		return 0;
	case I2C_TENBIT:
		if (arg)
    ...
    case I2C_RDWR:
		...
		//laputa test only return value checked 030830
		DbgMsg(LAPUTA_DEBUG_MSG,printk("+++> receive data %s\n",rdwr_arg.msgs->buf));
		...
		//laputa test only error checked 030830
		DbgMsg(LAPUTA_DEBUG_MSG,printk("+ res =  %d\n",res));
		...
		//laputa test only error check 030830
		DbgMsg(LAPUTA_DEBUG_MSG,printk("++++ res =  %d\n",res));
