...
#define io_base(port)	(*(volatile u_char *)(vCF_IO_BASE + port))
...
static int setup_time = 5; /* 80ns (no spec) */
static int cmd_time = 20;  /* 320ns (by spec, 25Mhz clock) */
static int recov_time = 5; /* 80ns (no spec) */
#else
static int has_dma = -1;
static int has_led = -1;
static int has_ring = -1;
...
#ifdef CONFIG_S3C2410_SMDK
	io_base(port) = (u_char)val;
	io_base(port+1) = (u_char)data;
#else
...
#ifdef CONFIG_S3C2410_SMDK
    /* Use INTR signal as the Management Interrupt on SMDK2410. */
    reg = 0;
#else
...
#ifdef CONFIG_S3C2410_SMDK
    if (io->start >= vCF_IO_BASE) io->start -= vCF_IO_BASE;
    if (io->stop >= vCF_IO_BASE) io->stop -= vCF_IO_BASE;
#endif
...
#ifdef CONFIG_S3C2410_SMDK
    mem->sys_start += pCF_MEM_BASE;
    mem->sys_stop += pCF_MEM_BASE;
#endif
...
#ifdef CONFIG_S3C2410_SMDK
    if (mem->sys_start >= pCF_MEM_BASE) mem->sys_start -= pCF_MEM_BASE;
    if (mem->sys_stop >= pCF_MEM_BASE) mem->sys_stop -= pCF_MEM_BASE;
#endif
...
static int __init init_i82365(void)
{
    ...
    set_external_irq(IRQ_nCF_INS, EXT_FALLING_EDGE, GPIO_PULLUP_EN);
    set_external_irq(IRQ_CF_RDY, EXT_RISING_EDGE, GPIO_PULLUP_EN);
#endif /* CONFIG_S3C2410_SMDK */
    ...
    if (grab_irq != 0)
#ifdef CONFIG_S3C2410_SMDK
	request_irq(cs_irq, pcic_interrupt, SA_INTERRUPT, "i82365", pcic_interrupt);
#else
