...
struct usb_hub *hub_old;

extern volatile int usb_error_flag;
...
#if 1
static void usb_hub_port_suspend(struct usb_hub *hub, int port)
{
	usb_set_port_feature(hub->dev, port + 1, USB_PORT_FEAT_SUSPEND);
}
#endif
...
// BUSHI //
	...
	if ((dev->devnum == 1)  // Root Hub
			&& hub->descriptor->bNbrPorts > CONFIG_MAX_ROOT_PORTS) {
		for (i=hub->descriptor->bNbrPorts-1; i>=0; i--) {
			printk("port #%d ", i);
			if (i > CONFIG_MAX_ROOT_PORTS-1) {
				usb_hub_port_suspend(hub, i);
				printk("suspened!\n");
			} else {
				printk("alived!\n");
			}
...
#ifdef CONFIG_PM
struct pm_dev *usb_hub_pm_dev;


int usb_hub_resume() //dz
{
	int pid;
	
	usb_hub_reset(hub_old);

	
}


void usb_hub_suspend(void)
{
	int ret;
...
static int
usb_hub_pm_callback(struct pm_dev *pm_dev, pm_request_t req, void *data)
{
	int pid;

	if (req == PM_SUSPEND) {
		
		printk("dz: hub suspend \n");			
		usb_hub_suspend();
		
		
	} else if (req == PM_RESUME) 
	{
		printk("dz: hub resume \n");				
...
#if 0
	usb_hub_pm_dev = pm_register(PM_USER_DEV, 0,
					 usb_hub_pm_callback);
#endif					 
