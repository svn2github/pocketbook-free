...
void ZLNXImageData::setPosition(unsigned int x, unsigned int y) {
	...
	myX = x;
	myY = y;
	myPosition = myImageData + (x >> 2) + y * myScanline;
	myShift = (x & 3) << 1;
}
...
void ZLNXImageData::copyFrom(const ZLImageData &source, unsigned int targetX, unsigned int targetY) {
	...
	for(int j = 0; j < sH; j++)
		for(int i = 0; i < sW; i++) {
			...
			c = myImageData + (i + targetX) / 4 + myScanline * (j + (targetY - sH));
			s = ((i + targetX) & 3) << 1;
...
//fprintf(stderr, "convertImageDirectJpeg\n");
...
//fprintf(stderr, "convertImageDirectGif\n");
	...
	if(stringData.length() < 500 && ! lock_drawing) {
