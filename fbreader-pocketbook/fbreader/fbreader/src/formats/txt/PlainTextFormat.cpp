...
void PlainTextFormatDetector::detect(ZLInputStream &stream, PlainTextFormat &format) {
	...
	unsigned int stringsWithLengthLessThan82Counter = 0;
	unsigned int stringsWithLength60to82Counter = 0;
	...
	do {
		...
		for (const char *ptr = buffer; ptr != end; ++ptr) {
			...
			if (*ptr == '\n') {
				...
				if (currentLineLength <= 82) {
					++stringsWithLengthLessThan82Counter;
					if (currentLineLength >= 60) {
						++stringsWithLength60to82Counter;
					}
				}
				if (!currentLineIsEmpty) {
...

	{
		...
		if (break_override == 0) {
			...
			if (stringsWithLength60to82Counter < 0.3 * nonEmptyLineCounter) {
