...
//	Message(ICON_WARNING, (char*)fileName, (char*)pathPrefix(reader).c_str(), 2000);
...
bool XHTMLReader::readFile(const std::string &pathPrefix, const std::string &fileName, const std::string &referenceName) {
	...
	// note: fileName is relative to the root of the EPUB! -> get the path too!
	int index = fileName.rfind('/');

	myPathPrefix = pathPrefix+fileName.substr(0, index+1);
