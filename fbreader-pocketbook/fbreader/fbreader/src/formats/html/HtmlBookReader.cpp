...
// Denis //
extern bool PrcShowImage;
extern int PrcRecordNumber;
extern int PrcmyMaxRecordIndex;
// Denis //
...
// Denis //
	if (PrcShowImage) {
		int imageCounter = PrcRecordNumber;
		std::string id;
		ZLStringUtil::appendNumber(id, imageCounter - PrcmyMaxRecordIndex);
...
HtmlPreTagAction::HtmlPreTagAction(HtmlBookReader &reader, FBTextKind kind) : HtmlControlTagAction(reader, kind) {
}

void HtmlPreTagAction::run(const HtmlReader::HtmlTag &tag) {
	HtmlControlTagAction::run(tag);
// Denis //
	if (PrcShowImage) {
		int imageCounter = PrcRecordNumber;
		std::string id;
...
shared_ptr<HtmlTagAction> HtmlBookReader::createAction(const std::string &tag) {
	if (tag == "EM") {
		...
		if (myProcessPreTag) {
			return new HtmlPreTagAction(*this, PREFORMATTED);
		}
	} else if (tag == "TABLE") {
