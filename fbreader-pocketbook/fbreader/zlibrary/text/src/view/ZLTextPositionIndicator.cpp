...
std::string ZLTextView::PositionIndicator::textPositionString() const {
	std::string buffer;
//	myTextView.UncorrectPage=endTextIndex();
	myTextView.CursorParagraph = myTextView.endCursor().paragraphCursor().index();
	myTextView.CursorWord = myTextView.endCursor().wordNumber();
	myTextView.CursorPosition = sizeOfTextBeforeCursor();
//	glb_page = 1 + sizeOfTextBeforeCursor() / 2048;
	glb_page = myTextView.ViewPage;
	ZLStringUtil::appendNumber(buffer, glb_page);
	//ZLStringUtil::appendNumber(buffer, 1 + sizeOfTextBeforeCursor() / 2048);
	buffer += '/';
	if (myTextView.CountFlag==0) 
		ZLStringUtil::appendNumber(buffer, myTextView.AllPage);
		else
		{
		 buffer+='~';
	 	 ZLStringUtil::appendNumber(buffer, (1 + sizeOfTextBeforeParagraph(endTextIndex()) / 2048)*1.5);
	 	}		
//1 + sizeOfTextBeforeParagraph(endTextIndex()) / 2048);
