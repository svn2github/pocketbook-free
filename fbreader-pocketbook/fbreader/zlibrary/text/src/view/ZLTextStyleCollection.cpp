		...
		if ((idString != 0) && (name != 0)) {
			...
			if (booleanValue(attributes, "partial")) {
				...
				if (alignmentString != 0) {
					...
					} else if (strcmp(alignmentString, "right") == 0) {
			...
			// DmitryZ: by default, use monospace font for preformatted and code
			if (fontFamily == 0 && (id == PREFORMATTED || id == CODE)) {
				decoration->FontFamilyOption.setValue("Liberation Mono");
			}
...
ZLTextStyleCollection::ZLTextStyleCollection() :
	...
	struct stat st;
	std::string stylesname = "/mnt/ext1/system/config/styles.xml";
	if (stat(stylesname.c_str(), &st) == -1) {
		stylesname = ZLibrary::DefaultFilesPathPrefix() + "styles.xml";
	}
	fprintf(stderr, "using %s\n", stylesname.c_str());
	ZLTextStyleReader(*this).readDocument(stylesname);
