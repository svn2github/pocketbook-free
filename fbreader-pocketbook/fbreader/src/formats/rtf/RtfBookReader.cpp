...
void RtfBookReader::addCharData(const char *data, size_t len, bool convert) {
	if (myCurrentState.ReadText) {
		if (convert || myConverter.isNull()) {
			if (outpos+len > maxBufferSize) flushBuffer();
			if (outpos+len >= outsize) {
				outsize = outpos+len+1024;
				outbuffer = (char *) realloc(outbuffer, outsize);
			}
			memcpy(outbuffer+outpos, data, len);
			outpos += len;
			if (outpos > maxBufferSize) flushBuffer();
		} else {
			flushBuffer();
...
void RtfBookReader::flushBuffer() {
	...
	if (myCurrentState.ReadText) {		
		if (!myConverter.isNull()) {
			...
			myConverter->convert(newString, outbuffer, outbuffer+outpos);
			characterDataHandler(newString);
			newString.erase();
		} else {
			static std::string newString(outbuffer, outpos);
			characterDataHandler(newString);
			newString.erase();
		}
	}
